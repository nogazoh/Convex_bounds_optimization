...
def solve_convex_problem_smoothed_kl_331(
    ...
    - If normalize_D=True: columns of D are normalized to sum_i D[i,t]=1.
    """
    ...
    # --------------------------------------------------------
    # Expected weighted losses L_mat[j,t]
    # --------------------------------------------------------
    L_mat = calculate_weighted_constraint_matrix(
        ...
        num_sources=k,
        loss_type=loss_type,
        normalize_D_cols=False,   # D already normalized above
    )
    ...
    # via -rel_entr(w_j, Q_ij) = w_j * log(Q_ij / w_j)
    # --------------------------------------------------------
    main_terms = [
    ...
    for t in range(k):
        constraints.append(S_all[t, :] @ (L_mat[:, t] - eps_vec[t]) <= 0)
