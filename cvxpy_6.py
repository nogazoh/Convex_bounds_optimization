...
def solve_convex_problem_soft_kl_diagonal_fast(
    Y,
    D,
    H,
    eta=1e-2,
    ...
    """
    Solver 6: uniform KL-penalty version

        max_{w,Q,R}
            sum_{i,j} p_{i|j} log(q_{j|i} / w_j)
            - eta * sum_t kappa_t(Q,R)
    ...
    print(
        f"   >>> [CVXPY-SoftKL-Diag-Fast] Setup: "
    ...
    print(f"   >>> [CVXPY-SoftKL-Diag-Fast] Starting {solver_type} solve...")
    ...
    try:
        ...
        print(f"   !!! [CVXPY-SoftKL-Diag-Fast] Solver Exception: {e}")
        return None, None, None

    if prob.status not in ["optimal", "optimal_inaccurate"]:
        print(f"   !!! [CVXPY-SoftKL-Diag-Fast] Failed/Infeasible. Status: {prob.status}")
        return None, None, None

    kappa_vals = np.array([kt.value for kt in kappa_terms], dtype=float)
    total_kappa = float(np.sum(kappa_vals))

    print(f"   >>> [CVXPY-SoftKL-Diag-Fast] Solved! Weights: {np.round(w.value, 3)}")
    print(f"   >>> [CVXPY-SoftKL-Diag-Fast] Objective value: {prob.value:.6f}")
    print(f"   >>> [CVXPY-SoftKL-Diag-Fast] kappa*: {np.round(kappa_vals, 6)}")
    print(f"   >>> [CVXPY-SoftKL-Diag-Fast] total_kappa*: {total_kappa:.6f}")
