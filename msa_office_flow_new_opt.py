...
# --- NEW: Hardcoded Loss Matrix from Logs ---
# Order must match DOMAINS: [Amazon, DSLR, Webcam]
# Rows: Source Model, Cols: Target Data
FULL_L_MAT = np.array([
...
def calculate_expected_loss(Y, H, D, num_sources, eps=1e-12):
    """
    REPLACED: Returns the hardcoded error rates for the CURRENT active domains.
    It ignores Y, H, D calculations and slices FULL_L_MAT instead.
    """
    global CURRENT_ACTIVE_DOMAINS, FULL_L_MAT, DOMAINS

    # 1. Identify indices of current active domains in the global DOMAINS list
    #    Example: if active are ('amazon', 'webcam'), indices are [0, 2]
    if not CURRENT_ACTIVE_DOMAINS:
        # Fallback if not set (should not happen in main loop)
        print("[WARNING] CURRENT_ACTIVE_DOMAINS is empty. Assuming all domains.")
        indices = range(len(DOMAINS))
    else:
        indices = [DOMAINS.index(d) for d in CURRENT_ACTIVE_DOMAINS]

    # 2. Slice the Full Matrix (rows=source_subset, cols=target_subset)
    #    np.ix_ allows us to select specific rows and specific columns
    L_mat = FULL_L_MAT[np.ix_(indices, indices)]

    # 3. Sanity check for shape
    if L_mat.shape != (num_sources, num_sources):
        print(f"[ERROR] Hardcoded L_mat shape {L_mat.shape} mismatch with num_sources={num_sources}")
        # If mismatch, fallback to identity or zeros to prevent crash, though logic implies match
